...
function misp_get_user_options(){
	...
	$defaults = array( 'misp_debug' => false
...
function misp_get_site_options(){
	...
	if ( !is_array( $misp_site_options ) ){
		$misp_site_options = array();
	}
	$defaults = array( 'misp_hidden_sizes' => array()
		, 'cache_buster' => true
	);
...
function misp_url( $id, $iframe=false ){
	if ($iframe) {
		$misp_url = admin_url( 'admin-ajax.php' )
			. "?action=misp_ajax&misp-action=iframe&misp-id={$id}"
			. "&TB_iframe=true";
	}
	else {
		$misp_url = admin_url('upload.php')
			. "?page=misp-edit&misp-id={$id}";
	}
...
function misp_admin_media_scripts($post_type){
	...
	if ($post_type == "attachment") {
		wp_enqueue_script( 'misp'
			, MISP_PLUGINURL . 'apps/coffee-script.js'
			, array('underscore')
			, MISP_VERSION
		);
		...
		wp_enqueue_script( 'misp'
			, MISP_PLUGINURL . 'js/snippets/misp_enable_media.js'
			, array('media-views')
			, MISP_VERSION
			, true
		);
		wp_enqueue_style( 'misp'
			, MISP_PLUGINURL . 'css/misp-media.css'
			, NULL
			, MISP_VERSION
		);
	}
	wp_localize_script('misp'
		, 'mispL10n'
		, array('PTE' => __('Post Thumbnail Editor', MISP_DOMAIN)
...
function misp_admin_post_thumbnail_html( $content, $post_id ){
	...
	return $content .= '<p id="misp-link" class="hide-if-no-js"><a class="thickbox" href="'
		...
		. '">'
		. esc_html__( 'Post Thumbnail Editor', MISP_DOMAIN )
		. '</a></p>';
}
...
function misp_add_thickbox() {
	add_thickbox();
	wp_enqueue_script('misp-fix-thickbox',
...
function misp_media_row_actions($actions, $post, $detached){
	...
	$actions['misp'] = "<a href='${misp_url}' title='"
		. __( 'Edit Thumbnails', MISP_DOMAIN )
		. "'>" . __( 'Thumbnails', MISP_DOMAIN ) . "</a>";
	return $actions;
}
...
function misp_admin_menu(){
	add_options_page( __('Post Thumbnail Editor', MISP_DOMAIN),
		__('Post Thumbnail Editor', MISP_DOMAIN),
		'edit_posts',
		'misp',
		'misp_launch_options_page'
	);
	// The submenu page function does not put a menu item in the wordpress sidebar.
	add_submenu_page(NULL, __('Post Thumbnail Editor', MISP_DOMAIN),
		__('Post Thumbnail Editor', MISP_DOMAIN),
		'edit_posts',
		'misp-edit',
		'misp_edit_page'
...
function misp_edit_setup() {
	...
	$title = __( "Post Thumbnail Editor", MISP_DOMAIN );
...
load_plugin_textdomain( MISP_DOMAIN
	, false
	, basename( MISP_PLUGINPATH ) . DIRECTORY_SEPARATOR . "i18n" );
