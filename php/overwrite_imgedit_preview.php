 ...
 */
function misp_wp_ajax_imgedit_preview() {
	$post_id = intval($_GET['postid']);
	if ( empty( $post_id ) || !misp_check_id( $post_id ) )
		wp_die( -1 );

	check_ajax_referer( "image_editor-$post_id" );

	include_once( ABSPATH . 'wp-admin/includes/image-edit.php' );
	if ( ! misp_stream_preview_image($post_id) )
		wp_die( -1 );
 ...
 */
function misp_stream_preview_image( $post_id ) {
	...
	$ratio = misp_image_get_preview_ratio( $w, $h );
	$w2 = $w * $ratio;
	$h2 = $h * $ratio;
