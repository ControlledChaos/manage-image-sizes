(function($) {
   ...
   var mispIframeLink;
   mispIframeLink = _.template(mispL10n.url + "&title=false");

   $( function() {
     var injectTemplate, template;
     // Change the attachment-details html
     injectTemplate = _.template("<a class=\"misp\" href=\"" +
                                 mispL10n.fallbackUrl.replace(/&TB_iframe=true/,'') +
                                 "\">\n   " +
                                 mispL10n.PTE +
                                 "\n</a>")({id:"{{ data.id }}"});
   ...
   wp.media.view.Attachment.Details = oldDetails.extend({
      ...
      events: _.extend(oldDetails.prototype.events, {
         'click .misp': 'loadPteEditor'
      }),

      // this.controller == wp.media.view.MediaFrame.Post instance
      // This sets/gets a state 'misp', that we can then modify to a state
      ...
      loadPteEditor: function(evt) {
         ...
         var me = this;
         if ( !featuredImageOpen && state !== "misp" ) {
         ...
         this.controller.state( 'misp' ).set({
            src: mispIframeLink({id:this.model.id}),
            title: mispL10n.PTE + ": " + this.model.attributes.filename,
            content: 'iframe'
         });

         if (this.controller.state().id == "misp") this.controller.state().trigger("activate");
         else this.controller.setState('misp');
   ...
   //        this.on( 'content:create:misp', this.mispLoadIframe, this );
   //    },
   //    mispLoadIframe: function( content ) {
   //        console.log( content );
   //        //console.log( this );
