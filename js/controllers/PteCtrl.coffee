define [
   ...
   app.controller "PteCtrl", ['$scope','$resource','$log', '$filter', ($scope, $resource, $log, $filter) ->
      ...
      $scope.thumbnailResource = $resource settings.ajaxurl,
         'action': 'misp_ajax'
         'misp-action': 'get-thumbnail-info'
      ...
      $scope.updateOptions = (update_options) ->
         update_options['misp-action'] = 'change-options'
         update_options['misp-nonce'] = settings.options_nonce
      ...
      $scope.save = (thumbnail) ->
         ...
         data =
            'misp-action': 'confirm-images'
            'misp-nonce': nonces['misp-nonce']
            id: id
         thumbnail_array = []
         if !thumbnail
         ...
         for thumbnail in thumbnail_array
            key = 'misp-confirm['+thumbnail.name+']'
      ...
      deleteTemp = ->
         if not nonces?['misp-delete-nonce']?
            return
         deleteResults = $.ajax settings.ajaxurl,
            ...
            data:
               'action': 'misp_ajax'
               id: id
               'misp-action': 'delete-images'
               'misp-nonce': nonces['misp-delete-nonce']
         return
      ...
      $scope.$watch 'currentThumbnailBarPosition', (x,y) ->
         ...
         $scope.updateOptions
            'misp_thumbnail_bar': $scope.currentThumbnailBarPosition
