define [
   ...
   app.controller "CropCtrl", ['$scope','$log', ($scope, $log) ->
      $scope.$watch 'mispCropSave', (x,y) ->
         if x is y
            return

         $scope.updateOptions
            'misp_crop_save': if $scope.mispCropSave then 'true' else 'false'
      ...
      $scope.$watch 'aspectRatio', ->
         ...
         if ar is null
            ...
            # Set the fit to crop automatically
               ...
               $scope.mispFitCrop = true
               $scope.mispFitCropColor = settings.i18n.transparent
         jcrop.setOptions
            aspectRatio: ar
         return
      ...
      $scope.toggleOptions = ->
         ...
         if !$scope.cropOptions
            ...
            $scope.mispJpgCompression = null
            $scope.updateSelected()
         return

      $scope.fitToCrop = (e) ->
         if !$( "#mispFitCrop" ).prop('checked')
            $scope.userDisabledFitCrop = true
            $scope.setInfoMessage $scope.i18n.crop_problems
            return

         color = $scope.mispFitCropColor || ""
         $("#misp-iris-dialog").find("input").val(color).iris
            hide: false
         .end().dialog
            ...
            buttons: [
               ...
               click: ->
                  $scope.$apply "mispFitCropColor = '"+ settings.i18n.transparent + "'"
                  $( this ).dialog( 'close' )
            ,
               text: settings.i18n.fitCrop_save
               click: ->
                  $scope.$apply "mispFitCropColor = '" + $( "input", this ).val() + "'"
                  $( this ).dialog( 'close' )
            ]
         return
      ...
      $scope.submitCrop = ->
         ...
         crop_options =
            'misp-action': 'resize-images'
            'id': settings.id
            'misp-sizes[]': selected_thumbs
            'w':w
            'h':h
            'x':x
            'y':y
            'misp-nonce': settings.nonce

         if $scope.mispCropSave
            crop_options['save'] = 'true'

         if 0 <= +$scope.mispJpgCompression <= 100
            crop_options['misp-jpeg-compression'] = $scope.mispJpgCompression

         # If the user wants to fit the crop to the thumbnail size, set the background color here
         if $scope.mispFitCrop
            color = $scope.mispFitCropColor || ""
            color = "" if color is settings.i18n.transparent
            crop_options[ 'misp-fit-crop-color' ] = color
         ...
         crop_results = $scope.thumbnailResource.get crop_options, ->
            ...
            $scope.setNonces
               'misp-nonce': crop_results['misp-nonce']
               'misp-delete-nonce': crop_results['misp-delete-nonce']
